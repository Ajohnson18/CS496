...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  let rec lookup_class' c_env =
    match List.head c_env with
    | (c_name,y) -> return y
    | (_, y) -> return lookup_class' (List.tail c_env) 
    | [] -> error "lookup_class: class c_name not found"
  in lookup_class' c_env
    
let new_env : string list -> env ea_result  = fun fs ->
  let rec new_env' = fun fs' ->
    match fs' with
      | h::t -> new_env' t >>+ 
                extend_env h 
                  (RefVal (Store.new_ref g_store (NumVal 0)))
      | [] -> return EmptyEnv
  in (new_env' (List.rev fs))
...
let lookup_method : string -> string -> class_env ->
  method_decl option = fun c_name m_name c_env ->
    (match (lookup_class c_name c_env) EmptyEnv with
      | Ok (_, _, m_env) -> (match List.assoc_opt m_name m_env with
                            | Some m -> Some m
                            | None -> None )
...
and
  ...
  (* SOOL operations *)
  | NewObject(c_name,es) ->
    mapM eval_expr es >>= fun args ->
    lookup_class c_name !g_class_env >>= fun (_, f, m) ->
    new_env f >>= fun env ->
    (match List.assoc_opt "initialize" methods with
    | Some m -> apply_method "initialize" (ObjectVal(c_name, env)) args m >>= fun _ ->
                return @@ ObjectVal(c_name, env)
    | None -> return @@ ObjectVal(c_name, env) )
  | Send(e,m_name,es) ->
    eval_expr e >>= fun self ->
    (match self with
    | ObjectVal(c_name, _) -> mapM eval_expr es >>= fun args ->
