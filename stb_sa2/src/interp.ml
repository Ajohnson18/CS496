...
let rec new_env : string list -> env ea_result  = fun fs ->
  let rec new_env' = fun f ->
    match f with
      | h::t -> new_env' t >>+ extend_env h (RefVal (Store.new_ref g_store (NumVal 1)))
      | [] -> return EmptyEnv
...
let lookup_method : string -> string -> class_env ->
  method_decl option = fun c_name m_name c_env ->
   (match (lookup_class c_name c_env) EmptyEnv with
      | Ok (id, v, m_env) -> 
        (match List.assoc_opt m_name m_env with
        | Some m -> Some m
        | None -> None )
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    new_env f >>= fun env ->
    (match List.assoc_opt "initialize" m with
