...
let rec rec_helper : Ast.decs -> string -> env -> 'a result =
  fun decs id env -> 
  match decs with
  | (s1,s2,body)::t -> if s1 = id then Ok (ProcVal (s2, body, env))
                          else rec_helper t id env
  | [] ->  Error "not found"
...
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec(decs, tail) ->
    match rec_helper decs id env with
    | Ok (ProcVal(x,y,z)) -> Ok (ProcVal(x,y,z))
    | _ -> apply_env id tail
